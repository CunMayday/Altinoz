# Website Content Guide for Dr. Cuneyt Altinoz

This document contains all the content from the current website to be used when rebuilding with a different design style.

## Brand Identity

### Name
**Dr. Cuneyt Altinoz**

### Tagline/Title
**Professor at Purdue University Global**

### Navigation Logo Options
- Short: "Professor Altinoz"
- Full: "Dr. Cuneyt Altinoz"

---

## Navigation Structure

The website should have the following sections in order:

1. **Home/Hero**
2. **Awards**
3. **Focus** (Current Projects)
4. **Education**
5. **Experience**
6. **Contact**

---

## Section 1: Hero/Home

### Main Heading
```
CUNEYT
ALTINOZ
```

### Subtitle Box
```
Professor at Purdue University Global
```

### Call-to-Action Buttons
1. **Primary Button**: "Get in Touch" → Scrolls to Contact section
2. **Secondary Button**: "Learn More" → Scrolls to Awards section

### Quick Stats Box

**Header**: "Welcome students. You are in good hands!"

**Statistics**:
- **16+** Years at Purdue
- **3** Advanced Degrees
- **5+** Major Awards

---

## Section 2: Awards & Recognition

### Section Title
**Awards & Recognition**

### Award Cards (4 total)

#### Award 1
- **Year**: 2025
- **Title**: Innovative Online Use of Technology
- **Organization**: Purdue University Online
- **Description**: Recognized for custom AI assistants helping graduate students
- **Icon**: Star

#### Award 2
- **Year**: 2023
- **Title**: Excellence in Online Course Design and Delivery
- **Organization**: Purdue University Online
- **Description**: Student-centric design of GB 513 Business Analytics
- **Icon**: Award/Trophy

#### Award 3
- **Year**: 2021
- **Title**: ACBSP Teaching Excellence Award
- **Organization**: ACBSP
- **Description**: One of 7 winners in the United States
- **Icon**: Trophy

#### Award 4
- **Year**: 2016
- **Title**: School of Business Professor of the Year
- **Organization**: Purdue University Global
- **Icon**: Star

### Leadership Roles Section

**Title**: Leadership Roles

**Role 1**:
- **Category**: Program Development
- **Description**: Faculty lead - Doctorate in Business Administration program

**Role 2**:
- **Category**: Faculty Advisor
- **Description**: Delta Mu Delta Honor Society

---

## Section 3: Current Focus

### Section Title
**Current Focus**

### Main Focus Statement
**Title**: AI Systems for Education

**Description**: Evaluation of developing AI systems for education - use of Gems, GPTs and webapps coded with AI assistance.

### Project Cards (3 total)

#### Project 1: Research Methodology Selector
- **Description**: Interactive tool for DBA students to identify appropriate research methodologies
- **URL**: https://cunmayday.github.io/webtests/methods.html
- **Button Text**: View Project

#### Project 2: Analytics Case Study
- **Description**: Comprehensive case study example demonstrating business analytics applications
- **URL**: https://cunmayday.github.io/webtests/caseada.html
- **Button Text**: View Project

#### Project 3: AI Use Cases Catalog
- **Description**: Curated collection of AI applications and use cases in education
- **URL**: https://usecases.vercel.app/index.html
- **Button Text**: View Project
- **Background Color**: Light grey (Purdue Gray #9D968D)

---

## Section 4: Education

### Section Title
**Education**

### Degree Cards (3 total)

#### Degree 1
- **Degree**: PhD
- **School**: North Carolina State University
- **Location**: Raleigh, NC
- **Years**: 1997-2001

#### Degree 2
- **Degree**: MBA
- **School**: Georgia Institute of Technology
- **Location**: Atlanta, GA
- **Years**: 1994-1996

#### Degree 3
- **Degree**: BS - Industrial Engineering
- **School**: North Carolina State University
- **Location**: Raleigh, NC
- **Years**: 1990-1994

---

## Section 5: Experience

### Section Title
**Experience**

### Position Cards (5 total)

#### Position 1: Professor at Purdue University Global
- **Title**: Professor
- **Organization**: Purdue University Global
- **Period**: 2009 - Present
- **Highlights**:
  - Faculty lead in charge of developing the new Doctorate in Business Administration program
  - Received the 2025 Purdue University Online award for Innovative Online Use of Technology
  - Received the 2023 Purdue University Online award for Excellence in Online Course Design
  - Received the 2021 ACBSP Teaching Excellence Award, one of 7 winners in the U.S.
  - Recognized as the 2016 School of Business Professor of the Year
  - Faculty advisor for Delta Mu Delta Honor Society

#### Position 2: Post-doctoral Research Fellow
- **Title**: Post-doctoral Research Fellow
- **Organization**: North Carolina State University
- **Period**: 2001 - 2003
- **Highlights**:
  - Created fuzzy logic based decision support system for supplier selection

#### Position 3: Research Assistant
- **Title**: Research Assistant
- **Organization**: North Carolina State University
- **Period**: 1997 - 2001
- **Highlights**:
  - Conducted research regarding decision support systems
  - Performance measurement in and of organizations
  - Simulations to analyze coordination
  - Workflow analysis of business processes
  - Support for business decisions using fuzzy logic
  - Developed two software programs
  - Research results led to opportunities for patents and business establishment

#### Position 4: Consultant
- **Title**: Consultant
- **Organization**: CIBA Vision, Atlanta GA
- **Period**: 1995 - 1997
- **Highlights**:
  - Consulted regarding inventory control issues
  - Developed and implemented new methodology for effective inventory control
  - Deliverables included inventory target levels and software tools
  - Expanded work to additional product lines after initial success
  - Analyzed shipping warehouse efficiency

#### Position 5: Operations Analyst
- **Title**: Operations Analyst
- **Organization**: IBM, Atlanta GA
- **Period**: 1995
- **Highlights**:
  - Analyzed service operations and developed measurement tools
  - Developed cost analysis tools and financial forecasts
  - Created new queries for information collection
  - Produced analysis reports and performance forecasts

---

## Section 6: Contact

### Section Title
**Let's Connect**

### Contact Information

#### Email
- **Label**: Email
- **Address**: caltinoz@purdueglobal.edu
- **Link**: mailto:caltinoz@purdueglobal.edu

### Social Links (Placeholder)
Note: Current implementation has placeholder links. Update these with actual social media profiles:
- **LinkedIn** → #
- **Twitter** → #
- **GitHub** → #

### Areas of Expertise (Tag Cloud)
- Business Administration
- AI in Education
- Decision Support Systems
- Inventory Control
- Industrial Engineering
- Online Education
- Educational Technology

### Collaboration Notice
**Text**: "Open to Collaboration & Research Opportunities"

### Footer
**Copyright**: © 2025 Cuneyt Altinoz. Built with neobrutalist vibes.
*(Note: Update "neobrutalist vibes" text based on new design style)*

---

## Design Color Palette (Purdue University Brand)

### Primary Colors
- **Campus Gold**: #C28E0E (Primary gold for highlights and accents)
- **Athletic Gold**: #CEB888 (Softer gold for secondary elements)

### Supporting Colors
- **Purdue Black**: #000000 (Primary text and strong accents)
- **Gray**: #9D968D (Supporting text and borders)
- **Dark Gray**: #373A36 (Secondary text)
- **White**: #FFFFFF (Clean backgrounds and contrast)

### Color Usage Guide
- **Primary accent color**: Campus Gold (#C28E0E)
- **Secondary accent color**: Athletic Gold (#CEB888)
- **Text primary**: Purdue Black (#000000)
- **Text secondary**: Dark Gray (#373A36)
- **Borders and supporting elements**: Gray (#9D968D)
- **Backgrounds**: White (#FFFFFF)

---

## Typography Preferences

Current website uses:
- **Display font**: Montserrat (bold, uppercase for headings)
- **Body font**: Space Grotesk
- **Monospace font**: Space Mono

*Adjust fonts based on new design style while maintaining readability and professional appearance.*

---

## Interactive Elements

### Scroll Behavior
- Navigation links should smooth scroll to corresponding sections
- "Learn More" button should scroll to Awards section
- "Get in Touch" button should scroll to Contact section

### Button Styles
- Primary buttons (for main CTAs)
- Secondary/outline buttons (for alternative actions)
- External link buttons (for project cards)

### Animation Considerations
- Cards should have hover effects
- Consider entrance animations for sections as they come into view
- Maintain smooth transitions between sections

---

## Assets Needed

### Icons
- Award/Trophy icons for awards section
- Star icons for awards section
- Users/people icon for leadership section
- Graduation cap icon for education
- Calendar icon for dates
- Map pin icon for locations
- Briefcase icon for experience
- Mail icon for contact
- Social media icons (LinkedIn, Twitter, GitHub)

### Images
- Optional: Professional headshot photo
- Optional: University/institution logos
- Optional: Background patterns or decorative elements

---

## Technical Requirements

### Sections IDs for Navigation
- `#hero` - Home/Hero section
- `#awards` - Awards & Recognition section
- `#focus` - Current Focus section
- `#education` - Education section
- `#experience` - Experience section
- `#contact` - Contact section

### External Links
1. https://cunmayday.github.io/webtests/methods.html
2. https://cunmayday.github.io/webtests/caseada.html
3. https://usecases.vercel.app/index.html
4. mailto:caltinoz@purdueglobal.edu

### Responsive Design
- Mobile-first approach
- Desktop navigation should show all links
- Mobile navigation should use hamburger menu
- Cards should stack on mobile, grid on desktop
- Text should scale appropriately for different screen sizes

---

## SEO & Meta Information

### Page Title
Dr. Cuneyt Altinoz - Professor & Supply Chain Expert

### Meta Description
Portfolio of Dr. Cuneyt Altinoz, Professor at Purdue University Global specializing in supply chain management, business administration, and decision support systems.

### Keywords (Suggested)
- Professor
- Purdue University Global
- Business Administration
- AI in Education
- Supply Chain Management
- Decision Support Systems
- Educational Technology
- Online Education
- Industrial Engineering

---

## Content Hierarchy

### Primary Message
Dr. Cuneyt Altinoz is an award-winning professor at Purdue University Global with expertise in business administration, AI in education, and decision support systems.

### Supporting Messages
1. Extensive academic credentials (PhD, MBA, BS)
2. Multiple teaching excellence awards
3. Leadership in program development
4. Innovative use of AI in education
5. Diverse professional experience in industry and academia

### Call-to-Action Priority
1. Contact via email (primary)
2. View current projects (secondary)
3. Connect on social media (tertiary)

---

## Notes for Redesign

1. **Current Style**: Neobrutalist design with bold borders, rotated elements, and flat colors
2. **Adaptability**: All content can work with any design style (minimalist, corporate, creative, etc.)
3. **Consistency**: Maintain Purdue University brand colors throughout
4. **Accessibility**: Ensure sufficient color contrast and readable typography
5. **Performance**: Optimize for fast loading times
6. **Mobile**: Prioritize mobile experience

---

*Last Updated: November 2025*
